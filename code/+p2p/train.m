function p2pModel = train(grayImage, colorImage, options)
...
%
% Args:
%   inData  - Training data input images (gray scale)
%   outData - Training data target images (color scale)
...
% Copyright 2020 The MathWorks, Inc.
    ...
    if ~isempty(options.CheckpointPath)
        ...
        % fullfile('folder', 'subFolder', 'fileName')
        mkdir(checkpointSubDir)
    end
    
    % option設定箇所(orファイル)がないため、手動で設定
    combinedChannels = 3 + 1;
    
    % model learns A to B mapping
    pairedImages = p2p.data.PairedImageDatastore(grayImage, colorImage, options.MiniBatchSize, ...
    ...
    if isempty(options.ResumeFrom)
        g = p2p.networks.generator(options.InputSize, 1, 3, options.GDepth);
        ...
        startEpoch = floor(iteration/pairedImages.NumObservations)+1;
    end
    
    %% Training loop
    for epoch = startEpoch:options.MaxEpochs
        
        pairedImages = pairedImages.shuffle();
        
        while pairedImages.hasdata
            
            iteration = iteration + 1;
            
            data = pairedImages.read();
            thisInput = cat(4, data.A{:});
